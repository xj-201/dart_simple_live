...
/// 自定义密码房间异常类
class NeedPasswordException implements Exception {
  final String message;
  NeedPasswordException([this.message = "该直播间已被设为加密房间，请输入密码"]);
  @override
...
class HuyaSite implements LiveSite {
  ...
  Future<LiveRoomDetail> getRoomDetail({required String roomId, String? password}) async {
    var roomInfo = await _getRoomInfo(roomId, password: password);
    
    // 判断密码校验是否正确或需要密码
    if (roomInfo["isPassWordRoom"] == 1 || roomInfo["isPassword"] == true) {
      if (password == null || password.isEmpty) {
        throw NeedPasswordException("该直播间已被设为加密房间，请输入密码");
      }
      if (roomInfo["passwordValid"] == false) {
        throw NeedPasswordException("密码错误，请重新输入");
      }
    }
    ...
    if (tLiveInfo == null || tProfileInfo == null) {
      throw Exception("获取虎牙直播间信息失败或未开播");
    }
  ...
  Future<Map> _getRoomInfo(String roomId, {String? password}) async {
    ...
    // 检测是否包含加密标识
    if (resultText.contains('"isPassWordRoom":1') || resultText.contains('加密房间')) {
      jsonObj["isPassWordRoom"] = 1;
    }
    if (resultText.contains("密码错误")) {
      jsonObj["passwordValid"] = false;
    }
